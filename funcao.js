...
function quanti(quant) {
  ...
  for (let i = 1; i <= quant; i++) {
    console.clear();
    do {
      console.log(`Escolha o sabor do ${i}º pão:\n\n(1) Calabresa\n(2) Azeitona\n(3) Tomate Seco\n(4) Parmesão\n`);
...
var quant = parseInt(prompt(`Quantos pães você gostaria de encomendar, ${nome}? `));
var escolhaP = quanti(quant);
console.log(`\nRESUMO DO PEDIDO DE ${nome.toUpperCase()}:\n\n`,escolhaP,`\n\nQuantidade de pães: `,quant);
const data = escolhaP.map((item) => item.preco).reduce((anterior, atual) => anterior + atual, 0);
